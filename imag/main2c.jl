...
function mkslid(irang, iinit)  
  ...
  gr = GtkGrid()
  gr[2,1] = Aslid; gr[1,1] = GtkLabel("alfa")
  ...
  can.draw = function(_)
      buf = fill(argb(Aslidval[], Rslidval[], Gslidval[], Bslidval[]), cx, cy)
      image(getgc(can), CairoARGBSurface(buf), 0, 0, cx, cy)
  end


  win = GtkWindow("Adjust", cx, 2cy)
  push!(win, gr)
  showall(win)
